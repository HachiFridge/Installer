...
fn detect_fridgerator_version() {
    println!("cargo:rerun-if-env-changed=FRIDGERATOR_VERSION");
    println!("cargo:rerun-if-changed=fridgerator.dll");

    // Allow manual override
    if std::env::var("FRIDGERATOR_VERSION").is_ok() {
        return;
    }

    let map = pelite::FileMap::open("fridgerator.dll").expect("fridgerator.dll in project root");
    let version_info = read_pe_version_info(map.as_ref()).expect("version info in fridgerator.dll");
    println!(
        "cargo:rustc-env=FRIDGERATOR_VERSION={}",
...
fn main() {
    // only set FRIDGERATOR_VERSION env at build time if net_install disabled
    if std::env::var("CARGO_FEATURE_NET_INSTALL").is_err() {
        detect_fridgerator_version();
    }
