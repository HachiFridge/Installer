...
impl Installer {
    ...
    pub fn get_target_display_label(&self, target: Target) -> String {
        ...
        if let Some(version_info) = self.get_target_version_info(target) {
            if version_info.is_fridgerator() {
                let name = version_info.name.as_deref().unwrap_or("Fridgerator");
    ...
    pub fn get_fridgerator_installed_target(&self) -> Option<Target> {
    ...
    pub fn install(&self) -> Result<(), Error> {
        ...
        #[cfg(feature = "net_install")]
        {
            ...
            let guard = self.fridgerator_dll.lock().unwrap();
            match guard.as_ref() {
                Some(Ok(bytes)) => {
        ...
        {
            mod_dll = include_bytes_zstd!("fridgerator.dll", 19);
        }
...
impl TargetVersionInfo {
    pub fn is_fridgerator(&self) -> bool {
        if let Some(name) = &self.name {
            return name == "Fridgerator";
        }
        false
    }
}
