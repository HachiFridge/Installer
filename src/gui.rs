...
pub fn run() -> Result<(), String> {
    ...
    #[cfg(feature = "net_install")]
    {
        ...
        let client = reqwest::blocking::Client::builder()
            .user_agent("fridgerator-installer")
            .build()
            .unwrap();
    ...
    #[cfg(feature = "net_install")]
    {
        let dll_handle = installer.fridgerator_dll.clone();
        thread::spawn(move || {
            let result = reqwest::blocking::get("https://github.com/HachiFridge/Fridgerator/releases/latest/download/fridgerator.dll")
    ...
    match message {
        ...
        WM_COMMAND => {
            ...
            match control_id {
                ...
                IDC_INSTALL => {
                    let installer = get_installer(dialog);
                    if let Some(target) = installer.get_fridgerator_installed_target() {
